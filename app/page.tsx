...
// --- 🚀 ORIGINAL PREMIUM BRAND SVGS (Pixel Perfect) ---
...
const Claude_Icon = () => <svg viewBox="0 0 24 24" width="24" height="24" fill="#D97757"><path d="M12 2L13 9L20 7L15 12L22 15L15 16L18 22L12 18L6 22L9 16L2 15L9 12L4 7L11 9L12 2Z"/></svg>;
const Gemini_Icon = () => <svg viewBox="0 0 24 24" width="20" height="20"><defs><linearGradient id="geminiGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#8AB4F8"/><stop offset="100%" stopColor="#4E7CFF"/></linearGradient></defs><path fill="url(#geminiGrad)" d="M12 2l2.5 7.5L22 12l-7.5 2.5L12 22l-2.5-7.5L2 12l7.5-2.5L12 2z"/></svg>;
    ...
    <div className="bg-[#1C1D21] min-h-screen relative text-[#EDEDED] font-sans selection:bg-orange-500/30 overflow-x-hidden">
      ...
      <nav className="relative z-50 flex items-center justify-between px-6 md:px-12 py-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={() => router.push('/dashboard/support')} className="flex items-center gap-2 text-[10px] md:text-xs font-bold text-gray-300 hover:text-white transition-colors uppercase tracking-widest">
      ...
      <section className="relative z-10 max-w-[1600px] mx-auto px-6 pt-4 pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20 items-start">
          ...
          <div className="flex flex-col items-center text-center lg:pt-8 xl:pt-12 w-full">
            {/* 🚀 Exact Spelling & Casing from Screenshot 1 */}
            ...
            <div className="flex flex-wrap justify-center items-center gap-3 mb-10 w-full max-w-2xl px-2">
              <button 
                ...
                className={`bg-white rounded-full px-5 py-2.5 flex items-center gap-2 shadow-xl transition-all duration-300 hover:scale-105 ${activeModel === 'gpt-5.2' ? 'ring-[3px] ring-blue-500 scale-105' : ''} ${isTokenSaved && activeModel !== 'gpt-5.2' ? 'opacity-30 pointer-events-none' : ''}`}
              >
                <OpenAI_Icon />
                <span className="text-[#10A37F] font-semibold text-lg tracking-tight">gpt-5.2</span>
              </button>

              <button 
                onClick={() => !isTokenSaved && setActiveModel("claude")} 
                className={`bg-white rounded-full px-4 py-2 flex items-center gap-2 shadow-xl transition-all duration-300 hover:scale-105 ${activeModel === 'claude' ? 'ring-[3px] ring-blue-500 scale-105' : ''} ${isTokenSaved && activeModel !== 'claude' ? 'opacity-30 pointer-events-none' : ''}`}
              >
                <Claude_Icon />
                <div className="text-left flex items-center gap-1.5">
                  <span className="text-[#D97757] font-semibold text-lg tracking-tight">Claude</span>
                  <span className="text-[#D97757] text-[10px] font-bold leading-none mt-1">Opus<br/>4.6</span>
                </div>
              </button>

              <button 
                onClick={() => !isTokenSaved && setActiveModel("gemini")} 
                className={`bg-white rounded-full px-5 py-2.5 flex items-center gap-2 shadow-xl transition-all duration-300 hover:scale-105 ${activeModel === 'gemini' ? 'ring-[3px] ring-blue-500 scale-105' : ''} ${isTokenSaved && activeModel !== 'gemini' ? 'opacity-30 pointer-events-none' : ''}`}
              >
                <Gemini_Icon />
                <span className="text-[#648AF5] font-semibold text-[17px] tracking-tight">Gemini 3 flash</span>
              </button>

              {/* Soon Button exactly in line */}
              <div className={`bg-white rounded-full px-5 py-2.5 flex items-center gap-2 shadow-xl cursor-not-allowed opacity-90 ${isTokenSaved ? 'opacity-20' : ''}`}>
                <Soon_Icon />
                <span className="font-bold text-[17px] text-gray-800 tracking-tight">soon</span>
              </div>
            </div>

            {/* 🚀 Channel Selector (Rectangular) */}
            ...
            <div className="flex flex-wrap justify-center items-center gap-3 mb-10 w-full max-w-2xl px-2">
              <button 
                ...
                className={`bg-white rounded px-6 py-3 flex items-center justify-center gap-3 shadow-md transition-all duration-300 hover:scale-105 min-w-[140px] ${activeChannel === 'telegram' ? 'ring-[3px] ring-blue-500 scale-105' : ''} ${isTokenSaved && activeChannel !== 'telegram' ? 'opacity-30 pointer-events-none' : ''}`}
              >
                <Telegram_Icon />
              ...
              <button 
                ...
                className={`bg-white rounded px-6 py-3 flex items-center justify-center gap-3 shadow-md transition-all duration-300 hover:scale-105 min-w-[140px] ${activeChannel === 'whatsapp' ? 'ring-[3px] ring-green-500 scale-105' : ''} ${isTokenSaved && activeChannel !== 'whatsapp' ? 'opacity-30 pointer-events-none' : ''}`}
              >
                <WhatsApp_Icon />
                {/* 🚀 Lowercase w as per screenshot */}
              ...
              <button className={`bg-white rounded px-4 py-2 flex items-center justify-center gap-3 shadow-md cursor-not-allowed min-w-[140px] ${isTokenSaved ? 'opacity-20' : ''}`}>
                ...
                <div className="text-left flex flex-col leading-tight">
                  <span className="text-base text-gray-700">discord</span>
              ...
              <button className={`bg-white rounded px-4 py-2 flex items-center justify-center gap-3 shadow-md cursor-not-allowed min-w-[140px] ${isTokenSaved ? 'opacity-20' : ''}`}>
                <Instagram_Icon/>
                 <div className="text-left flex flex-col leading-tight">
                   <span className="text-base text-gray-700">instagram</span>
                   <span className="text-[11px] text-gray-500">coming soon</span>
                 </div>
              </button>
            </div>
              ...
              <AnimatePresence mode="wait">
                {botLink ? (
                  ...
                  <motion.div key="login" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full flex flex-col items-center">
                    {/* 🚀 Exact Spelling & Long Pill */}
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => signIn("google")} className="w-full bg-white text-[#4B6B8A] py-4 rounded-[2rem] flex items-center justify-center gap-4 text-[22px] font-serif tracking-tight shadow-xl">
                  ...
                  <motion.div key="action" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full flex flex-col items-center gap-4">
                    ...
                    {!isTokenSaved ? (
                      <button onClick={() => handleOpenIntegration(activeChannel)} className="w-full bg-white text-black font-bold py-4 rounded-[2rem] text-[18px] shadow-xl uppercase tracking-widest transition-transform hover:scale-[1.02]">
                        CONNECT {activeChannel} TO CONTINUE
                      </button>
                    ) : (
                      <button onClick={() => handleOpenPricing(activeChannel)} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-[2rem] text-[18px] shadow-[0_0_30px_rgba(37,99,235,0.4)] uppercase tracking-widest transition-transform hover:scale-[1.02] flex justify-center items-center gap-3">
      ...
      {/* 🚀 COMPARISON SECTION (Requirement FIXED: EXACT MATCH OF SCREENSHOT 2) */}
      ...
      <footer className="pt-24 pb-12 relative z-10 bg-[#141414]">
        <div className="max-w-6xl mx-auto px-10 text-left mb-24">
          ...
          <button onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })} className="bg-[#FFA87A] hover:bg-[#FF905A] text-black font-bold px-10 py-3 rounded text-lg transition-colors shadow-lg font-serif">
            learn more
          </button>
        </div>
