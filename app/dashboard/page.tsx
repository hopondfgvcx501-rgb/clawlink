...
// Initialize Supabase Client (Strict Environment Fallbacks)
 ...
 * Fully expanded structure for scale and proper readability.
 * ==============================================================================================
 */
    ...
    plans: [
      {
        ...
        inr: 20916,
        msgs: "Smart Matrix",
        desc: "Elite multi-persona integration. 3x Fallback.",
        accent: "#00BFFF",
        color: "text-[#00BFFF]",
      },
      {
        ...
        inr: 149999,
        msgs: "Zero Downtime",
        desc: "Ultimate auto-routing & global priority access.",
        accent: "#BA7517",
        color: "text-[#BA7517]",
        badge: "Yearly ⭐",
        isYearly: true,
...
export default function Dashboard() {
  ...
  // Strict Loading Gatekeeper - Defaults to TRUE to prevent UI jumps
  ...
  const fetchKnowledge = async () => {
    ...
    try {
      const res = await fetch(`/api/knowledge?email=${session.user.email}&t=${Date.now()}`, {
        cache: "no-store",
      });
  ...
  // Ultra-Fast Parallel Engine Initializer
  useEffect(() => {
    ...
    try {
      ...
      if (
        ...
        console.error("Timezone parsing skipped.");
    }
   ...
   * ==============================================================================================
   */
  const rawDbModel = String(
    userData?.ai_model ||
    userData?.current_model_version ||
  ...
  const triggerRazorpayPayment = async () => {
    ...
    try {
      ...
      rzp.on("payment.failed", () => {
        setIsRenewing(false);
        alert("Payment cycle interrupted.");
      });
      rzp.open();
    } catch (error) {
  ...
  const totalMsgs =
    (stats?.platformStats?.whatsapp || 0) +
    (stats?.platformStats?.telegram || 0) +
    (stats?.platformStats?.web || 0) +
                ...
                <div className={`grid gap-4 sm:gap-6 mb-4 mx-auto text-left ${gridColsClass}`}>
                  {currentPricing?.plans?.map((plan: any) => {
                    ...
                    return (
                      <div
                        key={plan.id}
                        onClick={() => !isRenewing && setSelectedRenewalPlan(plan.id)}
                      ...
                      >
                        {plan.badge && (
                          <div
                            className="pricing-badge absolute -top-3 left-1/2 -translate-x-1/2 text-white text-[9px] font-bold uppercase px-3 py-1 rounded-full tracking-widest"
                            style={{ "--badge-color": plan.accent } as React.CSSProperties}
                          >
                            {plan.badge}
                          </div>
                        )}
