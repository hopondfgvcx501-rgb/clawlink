...
import { Radio, Send, Users, Activity, MessageSquare, Zap, AlertCircle } from "lucide-react";
...
export default function BroadcastEngine() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.email) {
      // Fetch total unique users the bot has interacted with
  ...
  if (isLoading || status === "loading") {
    return <div className="min-h-screen bg-[#111] flex items-center justify-center"><Activity className="w-8 h-8 text-orange-500 animate-spin" /></div>;
  }

  return (
    <div className="min-h-screen bg-[#0A0A0B] text-white p-6 md:p-10 font-sans selection:bg-pink-500/30 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 bg-[#111] border border-white/5 rounded-[2rem] p-8 shadow-2xl relative">
            ...
            <textarea 
              ...
              className="w-full bg-[#1A1A1A] border border-white/10 rounded-2xl p-5 text-gray-200 focus:border-pink-500 focus:shadow-[0_0_20px_rgba(236,72,153,0.15)] focus:outline-none transition-all resize-none mb-6 font-sans text-lg"
            />
