 ...
 * ==============================================================================================
 * @file app/dashboard/telegram/users/page.tsx  <-- (FIXED THE FILE PATH)
 * @description Centralized CRM to view and manage all users interacting with the Telegram Bot.
 * 🚀 FIXED: Pointed to the correct Backend API (/api/telegram/subscribers).
 * 🚀 FIXED: Implemented Absolute Dropdown Menu for Filters.
 * 🚀 FIXED: Activated robust CSV Export.
...
export default function TelegramSubscribers() {
  ...
  const fetchSubscribers = async () => {
    if (status === "authenticated" && session?.user?.email) {
      try {
        // 🔥 CRITICAL FIX: Fetching from the API we built, NOT /api/crm/leads
  ...
  // 🚀 FIXED: Working CSV Export Logic
  const handleExportCSV = () => {
      if (filteredSubscribers.length === 0) {
          alert("⚠️ No data available to export! List is empty.");
          return;
      }
