...
// 🚀 CORE WIDGET LOGIC (Now safely separated)
function WidgetChatbox() {
  ...
  if (!email) {
    return <div className="h-screen flex items-center justify-center bg-[#0A0A0B] text-gray-500 font-mono text-xs">Invalid Widget Config</div>;
  }
    ...
    <div className="h-screen w-full bg-[#0A0A0B] text-white flex flex-col font-sans">
      ...
      <div className="p-3 bg-[#111] border-t border-white/10">
        ...
        <div className="text-center mt-2">
          <a href="https://clawlink.com" target="_blank" className="text-[9px] text-gray-500 uppercase tracking-widest font-bold hover:text-white transition-colors">Powered by ClawLink</a>
        </div>
      </div>
    </div>
  );
...
// 🚀 MAIN EXPORT: Wrapped in Suspense to fix Vercel Build Error
export default function Widget() {
  return (
    <Suspense fallback={<div className="h-screen w-full bg-[#0A0A0B] flex items-center justify-center text-blue-500"><Loader2 className="w-8 h-8 animate-spin"/></div>}>
      <WidgetChatbox />
    </Suspense>
  );
}